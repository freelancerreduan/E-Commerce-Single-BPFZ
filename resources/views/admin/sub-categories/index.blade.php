                                    ...
                                    @forelse ($subcategories as $key => $subcategory)
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                <b> {{ $subcategory->category->category_name }}</b>
                                            </td>
                                            <td class="align-middle">{{ $subcategory->subcategory_name }}</td>
                                            <td class="align-middle">
                                                <form action="{{ route('subcategory.destroy', $subcategory->id) }}" method="POST" class="text-nowrap">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('subcategory.edit', $subcategory->id) }}" class="btn btn-info btn-sm mx-1" target="_blank">Edit</a>
